...
SET 
  TOTAL_AMOUNT = (
    ...
    FROM 
      ORDER_BASE.T_SALES_ORDER_DETAIL 
    WHERE 
      ORDER_ID = ORDER_BASE.T_SALES_ORDER_HEADER.ORDER_ID
  );
UPDATE 
  ORDER_BASE.T_SALES_ORDER_HEADER 
SET 
  ITEM_COUNT = (
    ...
    FROM 
      ORDER_BASE.T_SALES_ORDER_DETAIL 
    WHERE 
      CUSTOMER_ID = ORDER_BASE.T_SALES_ORDER_DETAIL.CUSTOMER_ID
  );
UPDATE 
  ORDER_BASE.T_CUSTOMER 
SET 
  TOTAL_ORDERS = (
    ...
    FROM 
      ORDER_BASE.T_SALES_ORDER_HEADER 
    WHERE 
      CUSTOMER_ID = ORDER_BASE.T_CUSTOMER.CUSTOMER_ID
  );
RETURN 'SUCCESS';
END;
$$;
...
UPDATE 
  ORDER_BASE.T_CUSTOMER 
SET 
  LOYALTY_POINTS = TOTAL_AMOUNT_COUNT * PROMOTION_VALUE 
FROM 
  (
    SELECT 
      CUSTOMER_ID, 
    ...
    FROM 
      (
        ...
        FROM 
          ...
          INNER JOIN ORDER_XFRM.V_CUSTOMER AS cu ON soh.CUSTOMER_ID = cu.CUSTOMER_ID
      ) AS co 
...
WHERE 
  ORDER_BASE.T_CUSTOMER.CUSTOMER_ID = ulp.CUSTOMER_ID;
RETURN 'SUCCESS';
END;
$$;
